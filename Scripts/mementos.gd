...
func _ready() -> void:
	for i in range(Global.unlockedLevels + 1):
		buttons[i].visible = true
	if (Global.unlockedLevels > -1):
		get_node("MementoWall").texture = load("res://Artwork/memento_wall_act_" + str(Global.unlockedLevels + 1) + ".png")
...
func _on_conclusion_pressed() -> void:
	get_tree().change_scene_to_file("res://Scenes/Levels/Conclusion.tscn")
